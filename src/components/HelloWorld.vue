...
export default {
    ...
    methods: {
        ...
        // 时间转换成时间戳
        timestampToTime2 (D, T) {
            var time = D + " " + T.substr(0, 5);
            return new Date(time).getTime();
        },
        // 请求历史数据
        getChartData() {
            axios.get(this.historyUrl + "minute" + "&code=" + "BTC/USDT").then(res => {
                if (res.data.status == "200") {
                    res.data.data.forEach(val => {
                        this.bars.unshift({
                            close: Number(val.closingPrice),
                            open: Number(val.openingPrice),
                            high: Number(val.highestPrice),
                            low: Number(val.lowestPrice),
                            volume: Number(val.volume),
                            time: this.timestampToTime2(val.date, val.time)
                        })
                    })
                }
            }).catch(err => {})
            ...
            Datafeed.DataPulseUpdater = function(datafeed, updateFrequency) {
                ...
                var update = function() {
                    ...
                    for (var listenerGUID in that._subscribers) {
                        ...
                        var datesRangeLeft =
                            datesRangeRight -
                            that.periodLengthSeconds(resolution, 10);
                        ...
                        (function(_subscriptionRecord) {
                            ...
                            that._datafeed.getBars(
                                ...
                                function(bars) {
                                    ...
                                    if (
                                        !that._subscribers.hasOwnProperty(
                                            listenerGUID
                                        )
                                    ) {
                                        return;
    ...
    data() {
        return {
            historyUrl: "http://47.244.98.44/home/Ajax/index?pageSize=500&page=1&goodsType=",
            wsUrl: "ws://47.244.98.44:7272",
            currency1: "USD",
            currency2: "BTC",
            saved_chart: null,
            chart: null,
